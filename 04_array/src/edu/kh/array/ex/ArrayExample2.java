...
public class ArrayExample2 {
	...
	public void deepCopy() {
		...
		//2.System.arraycopy(원본배열, 원복복사 시작인덱스, 복사 배열, 복사배열의 삽입 시작 인덱스 번호, 복사길이)
  ...
  // 배열을 이용한 중복데이터 제거 +정렬
	...
	public void createLottoNumber() {
		...
		for(int i=0; i<lotto.length; i++) {
			//3) 1~45사이 난수 생성
			int random = (int)(Math.random()*45+1); // 1<= x < 46
			...
			for(int x = 0 ; x<i ; x++) {
				//6) 현재 생성된 나수와 같은 수가 앞쪽 요소에 있는지 검사
				if(random == lotto[x]){i--;break;}
				//i가 1씩 증가할때마다 난수가 하나 생성됨 
				// -> 중복값이 있으면 나수를 새로 하나 더 생성해야함
				// -> i값을 인위적으로 1감소시켜 난수 발생의 기회를 한번 더 주는 것
				
				
				//앞쪽 중복데이터를 발견한다면 남은값을 비교할 필요가없으니
				// -> 효율 향상을 위해서 중복검사하는 for문 종료
			}
		}
