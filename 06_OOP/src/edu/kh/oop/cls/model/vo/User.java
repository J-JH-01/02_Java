...
public class User {
	...
	// 속성 (필드)
	// 아아디, 비밀번호, 이름, 나이, 성별 (추상화 진행) -> 유저라면 공통적으로 가지고 있을 법 한 데이터들
	...
	/*
	 * - new 연산자를 통해서 객체를 생성할 때 생성된 객체의 필드값 초기화+지정된 기능을 수행하는 역할 가능
	 */
	// 기본생성자(-> 매개변수 없는 생성자)
	public User() {
		// 기능
	...
	public User(String userId, String userPw, String userName, int userAge, char userGender) {
		...
		//this() 생성자
		// -> 단순하게 이렇게 쓰면 기본 생성자 , 컨트롤 클릭해보면 암
		// 같은 클래스의 다른 생성자를 호출할 때 사용
		// 생성자 내에서 반드시 첫번째 줄에 작성해야 한다
		// 왜 사용?
		// 죽복 코드 제거, 코드 길이 감소, 재사용성 증가
		// 가독성이 좀 떨어지는 경우가 있다(단점)
		
		
		
		this(userId,userPw); // **반드시 생성자 첫줄에 작성해야함!!!!
//		this.userId = userId;
//		this.userPw = userPw;
	...
	//자바는 기본적으로 필드명,생성자명,메서드명,변수명의 중복을 허용하지않음
