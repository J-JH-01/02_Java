...
public class GameService {
	...
	public void playerTurn() {
		...
		while (x) {
			...
			case 1:
				if(Armo.size()<1 || playerHP<1 || dealerHP<1)return;
				if(playerfire()) {x=false; System.out.println("{실탄}이였습니다!");passTurn=true;break;}
				else {System.out.println("[공포탄]이였습니다!");bonusTurn=true;break;}
			case 2:
				if(Armo.size()<1 || playerHP<1 || dealerHP<1)return;
				if(dealerfire()) { System.out.println("{실탄}이였습니다!");bonusTurn=true;break;}
				else {x=false;System.out.println("[공포탄]이였습니다! ");passTurn=true;break;}
			default:
				System.out.println("잘못 입력하셨습니다. 다시 시도합니다\n");
				break;
			}
			if(Armo.size()<1 || playerHP<1 || dealerHP<1)x=false;
			else {
				if(bonusTurn)System.out.println("추가턴을 시작합니다\n");
				if(passTurn)System.out.println("차례를 넘깁니다\n");
