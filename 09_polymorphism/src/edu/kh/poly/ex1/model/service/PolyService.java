...
public class PolyService {
	...
	public void ex1() {
		...
		// 1) 자식 객체가 부모 객체로 변하였기 때문에 자신만의 고유한 필드,메서드를 사용할 수 없음
		...
		car2.setFuel("전기모터");
		car2.setWheel(4);
		//car2.BatteryCapacity(1000000);
		// 부모타입 참조변수 이기때문에 Tesla의 고유필드,메서드 사용불가능
		// The method BatteryCapacity(int) is undefined for the type Car
		...
		// 각 배열 요소 Car 타입 참조변수 여야한다
		
		arr[0] = car; // Car 주소, 같은타입임
		...
		// 다형성(업캐스팅): 부모타입 참조변수 arr[i]로 자식객체 참조할 수 있다
		
		
	}
	
	// 전달받은 car 또는 자식객체(Tesla,Spark)의 엔진,연료,바퀴갯수 출력메서드
	public void PrintCar(Car temp) {
		...
		// Car temp = s
		// 다형성의 업캐스팅 모양과 똑같다
		// -> temp라는 매개변수에 Tesla, Spark, Car 의 주소가 넘어와도 된다(업캐스팅)
	...
	public void ex2() {
		...
		// 객체가 어떤 타입(자료형)인지 검사
		// instanceof 연산자 : 객체의 자료형을 검사하는 연ㅅ나자
		// -> 참조하는 객체가 특정 자료형이거나 상속관계인지 확인
		
		//arr[i] -> Car타입 Tesla() 객체
		System.out.println( arr[1] instanceof Tesla); // true (본인객체 맞음)
	...
	public void ex3() {
		...
		Car c1 = new Tesla ("전기모터","전기", 4,5000000);
		...
		// -> 형변환된 객체를 하나 만들던가 아니면 필요시에 형변환해서 뽑아내던가 하면 되나봄
	}
	...
	public void ex4() {
		...
		//Spark s1 = (Spark)c1; // c1을 관계없는 Spark로 형변환 해보리기 
	...
	public void ex5() {
		...
		// c1.toString() 호출코드와
		// edu.kh.poly.ex1.model.dto.Car.toStirng() 메소드 코드를 연결
		// -> 정적 바인딩
