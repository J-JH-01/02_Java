...
//		Set<String> set = new TreeSet<String>();
		...
		// 전달받은 e가 Set에 있는지 true, 없으면 false
		...
		// 해시 함수 -> 해시 테이블 
		// Java 버전, JVM 환경, OS 환경에 따라 미묘하게 테이블이 바뀌긴함
	...
	public void method3() {
		...
		Person p4 = new Person("길순이",20,'여');
		
		// Set 객체를 생성 후 p1~p4 추가
		...
		for(Person p : personSet)System.out.println(p);
	...
	public void method4() {
		...
		while(lotto.size() < 6) {
			...
			lotto.add(random.nextInt(45) + 1); // 1~45사이
			// 중복제거는 알아서 되기에 필요가 없음
			// 오름차순 정렬또한 되고있음
		}
	...
	public void lottoNumberGenerator() {
		...
		for(int i=0 ; i<a ; i++) {
			...
			while(lotto.size() < 6) {
				...
				lotto.add(random.nextInt(45) + 1); // 1~45사이w
		...
		int k=1;
		for(Set list : lottoList) {
			System.out.printf("%d회 : " + list,k);
			System.out.println();
			k++;
		}
