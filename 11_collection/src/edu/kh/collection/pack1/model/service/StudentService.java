...
//	private List<Student> studentList = new LinkedList<Student>(); // 추가,수정,삭제에 효율적
	...
	public StudentService() { // 기본 생성자
		...
		studentList.add(new Student("고영희", 99, "경기도 고양시", 'F', 100));
		studentList.add(new Student("갱얼쥐", 3, "펫샵", 'M', 40));
		studentList.add(new Student("미나리", 5, "상록수길", 'M', 30));
		studentList.add(new Student("페이커", 29, "서울시 종각", 'M', 20));
	}
...
//		// *매개변수 타입이 Object == 모든 객체 매개변수로 전달 할 수 있음
...
//
//		//testList 에서 꺼내온 인덱스 요소가 Student 혹은 상속관계인지 검사
...
//	}
	 ...
	 * 
	 * @author jjh@naver.com << 작성자의 이름, 보통은 궁금한점 물어보라고 주소 적어놓는 경우가 많음
	 * 
	 */
	public void displayMenu() {
		...
		do {
			...
			try {
				menuNum = sc.nextInt();
				// 여기서 예외발생하면 menuNum은 여전히 0이고, 입력버퍼에 이상한값이 남아있는 상태
				System.out.println();
				...
				case 5:
					...
					/* 이름으로 검색(일치) */ break;
				case 6:
					searchName2();/* 이름으로 검색(포함) */ break;
				case 7:
					sortByAge();/* 나이순으로 정렬 */ break;
				case 8:
					sortByName();/* 이름순으로 정렬 */ break;
				case 0:
				...
				sc.nextLine(); // 입력버터페 남아있는 잘못된 코드 제거

				menuNum = -1; // 첫 반복 시 잘못입력하는 경우
				// menuNum이 0을 가지고 있어 종료되는데,
				// 이를 방지하기 위해 아무값이나 넣어준것
			}
	...
	public String addStudent() throws InputMismatchException {
		// 예외가 발생하면 트라이에서 잡아줘서 에러문구출력쪽으로 넘어가게 해줄 것
		...
		if (studentList.add(new Student(name, age, region, gender, score))) {
			...
			// (반환형) -> 제네릭<Student> 때문에 List에 추가 할 수 있는 객체 타입이 Student로 제한됨
	...
	public void selectAll() {
		...
		// 현재 메서드를 종료하고 호출한 곳으로 돌아감. // 단 반환값은 없다(void)
		...
		// 작성법
		// for(컬렉션 또는 배열에서 꺼낸 한개의 변수 요소를 저장할 변수 : 컬렉션명(or 배열명){}
	...
	/**
	 * 학생 정보 수정 메서드 -학생정보가 studentList에 있는지(인덱스 번호로)검사, 없다면 "입력된 학생 정보가 없습니다" 문자열 반환
	 * - 입력된 숫자가 0보다 작은지 검사,작다면 "음수는 입력할 수 없습니다" 문자열 반환 - studentList 범위 내 인덱스 번호인지
	 * 검사, 초과했다면 "범위를 넘어선 값을 입력할 수 없습니다" 문자열 반환 - 수정이 완료 되었다면, "000의 정보가 변경되었습니다"
	 * 문자열 반환
	 * 
	 * @return
	 */
	public String updateStudent() throws InputMismatchException {
		...
		// 3) studentList 범위 내 인덱스번호인지 검사
			...
			// 4) 만약 문자열을 입력한 경우 -> 예외처리 되어있음(throws)
		} else {
			// 수정 코드 작성
	...
	public String removeStudent() throws InputMismatchException {
		...
		// 리스트에서 idnex번째 요소를 제거
		// 이 때, 제거된 요소가 반환된다
		// * List 중간에 비어잇는 인덱스가 없게 하기 위해서
	 ...
	 * 문자열 입력받아 studentList에서 꺼내온 Student값에 포함되지는 문자열인지 검사
...
System.out.println("===========학생 검색(이름 부분 포함)===============");
		...
		for(Student std : studentList) {
			...
			// boolean String.contaigons(문자열)
			// String에 문자열이 포함되어있으면 true/ 없으면 false
			
			
			if( std.getName().contains(input)) { //이름이 일치하는 경우
				//이거 순서 중요함 홍길동이 길동을 품냐 길동이 홍길동을 품냐 치아
				System.out.println(std); //std.toString();
				flag=false;
			}
		}
	...
	public void sortByAge() {
		...
		//Student에 comparabale 인터페이스를 상속받아 
        // compaerTo()에 정의한대로 정렬됨(오름차순,내림차순)
...
//		Comparator<Student> nameComparator = Comparator.comparing(Student::getName);
		...
		// Student 클래스의 getName() 메서드를 가르키는 것
